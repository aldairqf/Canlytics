...
| `test_signal_coverage.py` | `build_signal_coverage_report` (parallel stats_all/stats_real per signal, sentinel scales to bit width, byte_aligned property, PDU1/PDU2 classification, one item per CAN ID when a PGN has multiple sources, cancellation, multiple active DBCs, j1939 mode, progress reporting) |

Total actual: **366 tests** (`Ran 366 tests ... OK`).
